...
function transform(LoanRequest[] loanRequests, LoanApproval[] loanApprovals)
    ...
    Loan[] approvedLoans = from var loadRequest in loanRequests
        join var loadApproval in loanApprovals
        on loadRequest.loanRequestId equals loadApproval.loanRequestId
        select transformLoanRequest(loadRequest, loadApproval);
    ...
    RegionPerformance[] regionPerformance = from var {region, loanType, grantedAmount, dayOfWeek, interest, date}
        in approvedLoans
